...
if __name__ == '__main__':
    ...
    folder_name = name.split('_Epoch')[0]

    path1 = './Baseline_Embed/'+ args.POI_MODEL_NAME + '/'
    path2 = './Embed/Result_Embed/' + dataset + '/' + folder_name + '/'
    category = pd.read_csv(path1 + 'category.csv', usecols=['geo_id', 'category'])
    inputs = torch.load(path2 + name + '.pt').to(device) # Enhanced baseline embed
    # inputs = torch.load(path1 + "poi_repr.pth").to(device) # Baseline embed
