...
|------------------|-------------|
| `Baseline_Embed/` | POI embeddings from baseline models (poi2vec, skipgram, ctle, teaser, tale, hier) on NY/SG/TKY and corresponding downstream task *POI recommendation dataset* ; prepare in advance or download from the provided link |
...
| `Downstream/` | Downstream task code and corresponding dataset(classification, trajectory prediction, clustering, etc.) |
...
- **Baseline POI embeddings**: Prepare the POI embedding vectors to be enhanced and place them under **`Baseline_Embed/`**.  The poi recommendation task datasets are in the each baseline folder respectively.
